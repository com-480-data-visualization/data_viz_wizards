	...
	constructor(svg_element_id) {
		...
		const color_scale = d3.scaleLog()
            ...
            .then((data) => {
              ...
              data.forEach((row) => {
                ...
                if (!isNaN(popularity)) {
                  country_to_popularity[country] = popularity;
                }
              });
              return country_to_popularity;
            });

		const map_promise = d3.json("https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json").then((world_data) => {
		...
		Promise.all([population_promise, map_promise]).then((results) => {
            ...
            map_data.forEach(country => {
                ...
                country.properties.popularity = country_to_popularity[countryName];
            });
        
            const popularities = Object.values(country_to_popularity);
            ...
            this.map_container.selectAll(".country")
                ...
                .style("fill", d => color_scale(d.properties.popularity || 0));
